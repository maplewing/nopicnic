...
// ── International: USPS + UPS via Shippo ──

// Curated display names for USPS international services.
...
// Worst-case transit days when a carrier doesn't return estimated_days.
const INTL_FALLBACK_DAYS = {
  // USPS
  ...
  // UPS (Shippo tokens)
  ups_express_plus_international:   2,
  ups_express_international:        3,
  ups_saver_international:          3,
  ups_expedited_international:      5,
  ups_standard_international:       7,
};

async function getAllInternationalRates(country, zip, weightOz) {
  const shippoRes = await fetch("https://api.goshippo.com/shipments/", {
    method: "POST",
    headers: {
      Authorization: `ShippoToken ${process.env.SHIPPO_API_KEY}`,
  ...
  console.log("Shippo intl rates:", JSON.stringify(
    allRates
      .filter((r) => r.provider === "USPS" || r.provider === "UPS")
      .map((r) => ({
        provider: r.provider, token: r.servicelevel?.token,
        name: r.servicelevel?.name, amount: r.amount, days: r.estimated_days,
      }))
  ));
  ...
  return allRates
    ...
    .map((r) => {
      ...
      const days = r.estimated_days || INTL_FALLBACK_DAYS[svcToken] || 14;
      // Use curated name for USPS; Shippo's own name for UPS.
      const service = USPS_INTL_SERVICE_NAMES[svcToken] || r.servicelevel?.name;
      return {
        token: r.object_id,
...
async function getInternationalRates(country, zip, weightOz) {
  const all = (await getAllInternationalRates(country, zip, weightOz))
    .sort((a, b) => parseFloat(a.amount) - parseFloat(b.amount));

  if (all.length === 0) return [];

  const cheapest = all[0];
  const faster = all.filter((r) => r.token !== cheapest.token && r.speedDays < cheapest.speedDays);

  const fastest = faster.length
    ? faster.slice().sort((a, b) => a.speedDays - b.speedDays || parseFloat(a.amount) - parseFloat(b.amount))[0]
    : null;

  const middle = fastest
    ? faster.find(
        (r) =>
          r.token !== fastest.token &&
          parseFloat(r.amount) < parseFloat(fastest.amount) &&
          r.speedDays <= cheapest.speedDays - 2
      ) || null
    : null;
  ...
  // Drop anything both slower and pricier than another option.
