...
// ── International: USPS (Shippo) + UPS (EasyPost) ──
...
async function getUPSInternationalRates(country, zip, weightOz) {
  ...
  if (!epRes.ok) {
    console.error("EasyPost error:", await epRes.text());
    return [];
  }
...
async function getInternationalRates(country, zip, weightOz) {
  const [uspsResult, upsResult] = await Promise.allSettled([
    getUSPSInternationalRates(country, zip, weightOz),
    getUPSInternationalRates(country, zip, weightOz),
  ]);

  const uspsRates = uspsResult.status === "fulfilled" ? uspsResult.value : [];
  const upsRates  = upsResult.status  === "fulfilled" ? upsResult.value  : [];
  if (uspsResult.status === "rejected") console.error("USPS intl failed:", uspsResult.reason);
  if (upsResult.status  === "rejected") console.error("UPS intl failed:",  upsResult.reason);

  const all = [...uspsRates, ...upsRates]
