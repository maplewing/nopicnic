...
const itemImage = (item) => (typeof item === "string" ? null : item.image || null);

// transactional: true omits the unsubscribe footer (for order-triggered emails).
// Anything else needs `email` so the opt-out link can be signed for that
// recipient — these go out through Resend, which does no merge-tag substitution,
// so a placeholder here would ship to the customer verbatim.
function shell(body, { transactional = false, email = null } = {}) {
  const optOut =
    !transactional && email
      ? `<br><a href="${unsubscribeUrl(email, siteUrl)}" style="color:#555555;">Unsubscribe</a>`
      : "";
  const footer = `<p style="${monoMuted}margin:16px 0 0;font-size:12px;">
         No Picnic Press · Berkeley, CA${optOut}
       </p>`;
...
export function announcementEmail(firstName = "there", email = null) {
  return shell(`
      ...
      ? `<p style="${label}margin:32px 0 6px;">What's inside</p>
         <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-top:2px solid #000000;margin-bottom:32px;">
           ${items.map((item) => {
             ...
             return img
               ? `<tr>
                   <td style="padding:8px 10px 8px 0;border-bottom:1px solid #e5e5e5;width:52px;vertical-align:middle;">
                     <table width="52" cellpadding="0" cellspacing="0" border="0"><tr><td width="52" height="52" style="width:52px;height:52px;background:url('${img}') center/cover no-repeat #f0f0f0;border-radius:2px;">&nbsp;</td></tr></table>
                   </td>
                   <td style="${mono}padding:8px 0;border-bottom:1px solid #e5e5e5;">${name}</td>
                 </tr>`
  ...
  const promoBlock = promoCode
    ? `<table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:32px;">
        <tr><td style="border-top:1px solid #e5e5e5;padding-top:28px;">
          <p style="${label}margin:0 0 12px;">A little something</p>
          <p style="${mono}margin:0 0 16px;">
            Here's 20% off your next order — for you or anyone you want to share it with.
          </p>
          <p style="font-family:'Courier New',Courier,monospace;font-size:22px;font-weight:bold;letter-spacing:0.1em;margin:0 0 12px;color:#000000;">${promoCode}</p>
          <p style="${monoMuted}margin:0 0 24px;">
            Valid through ${promoExpiry}. Use at checkout.
          </p>
          <a href="${siteUrl}" style="${btn}">Shop now →</a>
        </td></tr>
      </table>`
    : "";
...
export function shippingConfirmationEmail(
  ...
  return shell(`
    ...
    <p style="${mono}margin:0 0 16px;">Your package is en route from us to you. Tracking info is below.</p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
      <tr><td style="border-top:1px solid #e5e5e5;padding-top:28px;">
        <p style="${label}margin:0 0 16px;">From the blog</p>
        <table width="100%" cellpadding="0" cellspacing="0" border="0">
          <tr>
            ${recentPosts.slice(0, 2).map((post) => `
            <td style="width:50%;vertical-align:top;padding-right:10px;">
              ${post.image ? `<a href="${post.link}" style="display:block;margin-bottom:8px;text-decoration:none;"><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td height="160" style="height:160px;background:url('${post.image}') center/cover no-repeat #f0f0f0;">&nbsp;</td></tr></table></a>` : ""}
              <a href="${post.link}" style="${mono}font-size:13px;color:#000000;text-decoration:underline;">${post.title}</a>
            </td>`).join("")}
          </tr>
        </table>
      </td></tr>
    </table>` : suggestedProducts.length > 0 ? `
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
      <tr><td style="border-top:1px solid #e5e5e5;padding-top:28px;">
        <p style="${label}margin:0 0 16px;">You might also like</p>
        <table width="100%" cellpadding="0" cellspacing="0" border="0">
          <tr>
            ${suggestedProducts.slice(0, 2).map((p) => `
            <td style="width:50%;vertical-align:top;padding-right:10px;">
              ${p.image ? `<a href="${siteUrl}/shop/${p.slug}" style="display:block;margin-bottom:8px;text-decoration:none;"><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td height="160" style="height:160px;background:url('${p.image}') center/cover no-repeat #f0f0f0;">&nbsp;</td></tr></table></a>` : ""}
              <a href="${siteUrl}/shop/${p.slug}" style="${mono}font-size:13px;color:#000000;text-decoration:none;font-weight:bold;">${p.name}</a>
              ${p.subtitle ? `<p style="${mono}font-size:12px;color:#555555;margin:4px 0 0;">${p.subtitle}</p>` : ""}
            </td>`).join("")}
          </tr>
        </table>
      </td></tr>
...
export function reviewRequestEmail(firstName = "there", items = [], email = null) {
  const productName =
    items.length === 1
      ? itemName(items[0])
      : items.length > 1
      ? `${itemName(items[0])} and ${items.length - 1} other item${items.length > 2 ? "s" : ""}`
      : "your recent order";

  return shell(`
    <p style="${mono}margin:0 0 16px;">${firstName},</p>
    ...
    <p style="${mono}margin:0 0 32px;">
      ...
      and it helps us know what's working. Even a sentence is useful.
    </p>

    <a href="${siteUrl}/reviews" style="${btn}">Leave a review →</a>

    <p style="${monoMuted}margin:32px 0 0;">
      And if you have questions or thoughts you want to share privately, just reply here.
    </p>
  `, { email });
}
