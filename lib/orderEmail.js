...
// orderNumber: optional sequential order number (e.g. 2684)
export function orderConfirmationEmail(session, downloadLinks = [], orderNumber = null) {
  const name = session.customer_details?.name || "there";
  const address = session.shipping_details?.address;
  const items = session.line_items?.data || [];
  const discount = session.total_details?.amount_discount || 0;
  const shippingCost = session.shipping_cost?.amount_total;
  // Always use the production domain for email assets — localhost isn't reachable by email clients
  const siteUrl = "https://nopicnicpress.com";
  const downloadSiteUrl = process.env.NEXT_PUBLIC_URL || siteUrl;
  ...
  const itemRows = items
    .map((item) => {
      ...
      const thumb = item.price?.product?.images?.[0];
      return `
        <tr>
          ${thumb ? `<td style="padding:10px 12px 10px 0;border-bottom:1px solid #e5e5e5;width:52px;vertical-align:top;">
            <img src="${thumb}" alt="" width="52" height="52" style="display:block;object-fit:cover;border-radius:2px;border:0;">
          </td>` : ""}
          <td style="${mono}padding:10px 0;border-bottom:1px solid #e5e5e5;">
            ${item.description}${qty}
          </td>
          <td style="${mono}padding:10px 0;border-bottom:1px solid #e5e5e5;text-align:right;white-space:nowrap;vertical-align:top;">
  ...
  // Use downloadSiteUrl for download links so they work in local testing too
  const resolvedDownloadLinks = downloadLinks.map((dl) => ({
    ...dl,
    formats: dl.formats.map((f) => ({
      ...f,
      url: f.url.replace(siteUrl, downloadSiteUrl),
    })),
  }));
        ...
        <table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:520px;">
          <tr>
            <td>
              ...
              <p style="${mono}margin:0 0 4px;color:#555555;">Hi ${name}, thanks for your order.</p>
              ${orderNumber ? `<p style="${mono}margin:0 0 36px;font-size:11px;text-transform:uppercase;letter-spacing:0.08em;color:#aaaaaa;">Order #${orderNumber}</p>` : `<p style="${mono}margin:0 0 36px;"></p>`}
              ...
              ${resolvedDownloadLinks.length > 0 ? `
              ...
              ${resolvedDownloadLinks.map(({ name, formats }) => `
