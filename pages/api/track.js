...
const SAMPLE_RATE = 3; // record 1-in-3 requests, multiply counts by 3
...
export default async function handler(req, res) {
  ...
  // Sample: skip ~2 out of 3 requests
  ...
  try {
    ...
    if (page) {
      data.totalViews = (data.totalViews || 0) + SAMPLE_RATE;
      data.pages = data.pages || {};
      data.pages[page] = (data.pages[page] || 0) + SAMPLE_RATE;
    }

    if (event) {
      data.events = data.events || {};
      data.events[event] = (data.events[event] || 0) + SAMPLE_RATE;
      if (product) {
        data.events[`${event}:${product}`] = (data.events[`${event}:${product}`] || 0) + SAMPLE_RATE;
      }
    }

    if (sessionStart) {
      data.events["session_start"] = (data.events["session_start"] || 0) + SAMPLE_RATE;
    }

    if (sessionEngaged) {
      data.events["session_engaged"] = (data.events["session_engaged"] || 0) + SAMPLE_RATE;
    }
